...
// clang-format off
// Things have to be in this order because the libvlc headers have issues
...
media_subitemadded(libvlc_media_t* parent, libvlc_media_t* media)
{
    ...
    libvlc_event_attach(
        mem, libvlc_MediaSubItemAdded, media_event_handler, NULL);
    libvlc_event_attach(
        mem, libvlc_MediaStateChanged, media_event_handler, NULL);
...
media_event_handler(const struct libvlc_event_t* p_event, void* p_data)
{
    ...
    case libvlc_MediaSubItemAdded:
        media_subitemadded((libvlc_media_t*) p_event->p_obj,
            p_event->u.media_subitem_added.new_child);
        break;
    case libvlc_MediaStateChanged:
        media_statechanged((libvlc_media_t*) p_event->p_obj,
...
static void
media_list_player_event_handler(
...
play_media_list(libvlc_instance_t* vlc, libvlc_media_list_t* ml)
{
    ...
    libvlc_event_attach(mlpem, libvlc_MediaListPlayerPlayed,
        media_list_player_event_handler, NULL);
    libvlc_event_attach(mlpem, libvlc_MediaListPlayerStopped,
        media_list_player_event_handler, NULL);
    ...
    if (random_seek) {
        libvlc_media_player_t* mp
            = libvlc_media_list_player_get_media_player(mlp);

        for (int i = 0; i < 100; i++) {
            /* Random seek to between 0% and 90% of the file */
            libvlc_media_player_set_position(
                mp, (0.9f * (float) rand()) / (float) RAND_MAX);
...
play_media(libvlc_instance_t* vlc, libvlc_media_t* md)
{
    ...
    libvlc_event_attach(
        mdem, libvlc_MediaSubItemAdded, media_event_handler, NULL);
    libvlc_event_attach(
        mdem, libvlc_MediaStateChanged, media_event_handler, NULL);
    ...
    libvlc_event_attach(
        mpem, libvlc_MediaPlayerOpening, media_player_event_handler, NULL);
    libvlc_event_attach(
        mpem, libvlc_MediaPlayerBuffering, media_player_event_handler, NULL);
    libvlc_event_attach(
        mpem, libvlc_MediaPlayerPlaying, media_player_event_handler, NULL);
    libvlc_event_attach(
        mpem, libvlc_MediaPlayerPaused, media_player_event_handler, NULL);
    libvlc_event_attach(
        mpem, libvlc_MediaPlayerStopped, media_player_event_handler, NULL);
